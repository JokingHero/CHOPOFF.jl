using CHOPOFF.FMIndexes: FMIndex, restore, count, locate, locateall
using Combinatorics
using Test
using Random
...
@testset "FMIndexes" begin
    @testset "construct" begin
        ...
        @testset "use pSAscan" begin
            ...
            if haskey(ENV, "PSASCAN")
                ...
                @info "Skipped a testfor pascan as it's not available."
    ...
    @testset "full-text search" begin
        function linear_search_(query, textstr)
            locs = Int[]
            loc = 0
        ...
        text = read("./sample_data/lorem_ipsum.txt")
        ...
        for query in ["a", ".",
                      ...
                      "odio", "Cras",
                      "sollicitudin"]
            locs = linear_search_(query, textstr)
