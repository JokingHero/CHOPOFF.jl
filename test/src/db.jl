...
@testset "databases" begin
    ...
    @testset "linearDB vs prefixHashScan" begin
        # prefixHashScan is indexless, so linearDB is the only brute-force
        # anchor it has. Every other prefixHashScan test compares its backends
        # against each other, which cannot catch an error they all share.
        motif = Motif("Cas9")
